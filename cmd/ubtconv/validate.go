...
// ValidateDiffAgainstMPT validates the diff's account/storage values directly against MPT via RPC,
// without requiring a local UBT trie. Used in validate-only mode where diffs are not applied.
func (v *Validator) ValidateDiffAgainstMPT(blockNumber uint64, diff *ubtemit.QueuedDiffV1) error {
	client, err := v.reader.getClient()
	if err != nil {
		return fmt.Errorf("get RPC client for diff validation: %w", err)
	}
	...
	for _, acct := range diff.Accounts {
		...
		err := client.CallContext(ctx, &mptBalance, "eth_getBalance", acct.Address, blockNr)
		cancel()
		if err != nil {
			return fmt.Errorf("eth_getBalance(%s): %w", acct.Address, err)
		}
		if mptBalance.ToInt().Cmp(acct.Balance) != 0 {
			log.Warn("Diff validation: balance mismatch", "addr", acct.Address, "block", blockNumber,
				"diff", acct.Balance, "mpt", mptBalance.ToInt())
			mismatches++
		}

		ctx, cancel = context.WithTimeout(context.Background(), v.timeout)
		var mptNonce hexutil.Uint64
		err = client.CallContext(ctx, &mptNonce, "eth_getTransactionCount", acct.Address, blockNr)
		cancel()
		if err != nil {
			return fmt.Errorf("eth_getTransactionCount(%s): %w", acct.Address, err)
		}
		if uint64(mptNonce) != acct.Nonce {
			log.Warn("Diff validation: nonce mismatch", "addr", acct.Address, "block", blockNumber,
				"diff", acct.Nonce, "mpt", uint64(mptNonce))
			mismatches++
		}
	}
	...
	for _, slot := range diff.Storage {
		...
		err := client.CallContext(ctx, &mptValue, "eth_getStorageAt", slot.Address, slot.SlotKeyRaw, blockNr)
		cancel()
		if err != nil {
			return fmt.Errorf("eth_getStorageAt(%s, %s): %w", slot.Address, slot.SlotKeyRaw, err)
		}
		if mptValue != slot.Value {
			log.Warn("Diff validation: storage mismatch", "addr", slot.Address, "slot", slot.SlotKeyRaw,
				"block", blockNumber, "diff", slot.Value, "mpt", mptValue)
			mismatches++
		}
	}

	if mismatches > 0 {
		return fmt.Errorf("diff validation found %d mismatches at block %d", mismatches, blockNumber)
	}

	log.Debug("Diff validation passed", "block", blockNumber, "accounts", len(diff.Accounts), "storage", len(diff.Storage))
	return nil
}
