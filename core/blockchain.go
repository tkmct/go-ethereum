...
func (bc *BlockChain) insertChain(ctx context.Context, chain types.Blocks, setHead bool, makeWitness bool) (*stateless.Witness, int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		parentExecRoot := parent.Root
		if bc.binaryFollowEnabled() {
			parentExecRoot = bc.getCurrentExecRoot()
		}
		// The traced section of block import.
		start := time.Now()
		res, err := bc.ProcessBlock(ctx, parentExecRoot, block, setHead, makeWitness && len(chain) == 1)
		if err != nil {
			return nil, it.index, err
		}
...
func (bc *BlockChain) recoverAncestors(ctx context.Context, block *types.Block, makeWitness bool) (common.Hash, error) {
	...
	if bc.binaryFollowEnabled() {
		return bc.recoverBinaryAncestors(block)
	}
	// Gather all the sidechain hashes (full blocks may be memory heavy)
	var (
		hashes  []common.Hash
...
func (bc *BlockChain) recoverBinaryAncestors(block *types.Block) (common.Hash, error) {
