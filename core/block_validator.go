...
func (v *BlockValidator) ValidateState(block *types.Block, statedb *state.StateDB, res *ProcessResult, stateless bool) error {
	...
	if !v.bc.cfg.SkipStateRootValidation {
		if root := statedb.IntermediateRoot(v.config.IsEIP158(header.Number)); header.Root != root {
			return fmt.Errorf("invalid merkle root (remote: %x local: %x) dberr: %w", header.Root, root, statedb.Error())
		}
	} else if err := statedb.Error(); err != nil {
		// Even if the root check is disabled, a database error means execution was
		// performed on incomplete/corrupt state, so fail fast.
		return fmt.Errorf("state error with stateRoot validation disabled: %w", err)
	}
	return nil
}
